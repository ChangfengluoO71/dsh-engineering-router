// Modified by dsh-engineering-router: isolate persistent/global router state from router-standard.
/**
...
const stageFile = () => process.env.DSH_ROUTER_STAGE_FILE || join(dshHomeForState(), 'engineering-router', 'stages.json')
let stageCache = null
...
const sharedLift = globalThis[Symbol.for('engineering-router.restrictLift')] ?? (globalThis[Symbol.for('engineering-router.restrictLift')] = new Map())
/** 跨代共享 override：main 注册与 own-layer shim 必须读写同一张表，否则 status 看不到 mode 覆盖。 */
const overrideMap = () => globalThis[Symbol.for('engineering-router.overrides')] ??= new Map()
...
export function apply(ctx, config) {
  try { mkdirSync(join(dshHomeForState(), 'engineering-router'), { recursive: true }); writeFileSync(join(dshHomeForState(), 'engineering-router', 'last-mount.txt'), 'new-gen v0.8 ' + new Date().toISOString(), 'utf8') } catch { /* marker */ }
  ...
  try {
    ...
    try {
      ...
      writeFileSync(join(dshHomeForState(), 'engineering-router', 'bash-diag.json'),
