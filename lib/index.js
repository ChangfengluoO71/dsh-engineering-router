...
const SKILL_ID = 'engineering-project-bootstrap'
...
const SKILL_SOURCE = join(ROOT, 'skills', SKILL_ID)
...
export function apply(_ctx, config = {}) {
  ...
  let skill = { changed: false, reason: 'disabled' }
  if (config.installBootstrapSkill !== false) {
    skill = installManagedDirectory({
      source: SKILL_SOURCE,
      target: join(home, 'skills', SKILL_ID),
      label: SKILL_ID,
      required: ['SKILL.md'],
      force,
    })
  ...
  console.log('[' + name + '] ready: preset=' + (preset.changed ? 'updated' : preset.reason)
    ...
    + ' skill=' + (skill.changed ? 'updated' : skill.reason))
}
